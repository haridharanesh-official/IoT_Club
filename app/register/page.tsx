...
export default async function RegisterPage() {
  ...
  const { data: application } = await supabase.from('membership_applications').select('id').eq('user_id', user.id).maybeSingle()
  if (application) redirect('/membership/status')
  return <RegistrationForm collegeEmail={user.email ?? ''} />
}
