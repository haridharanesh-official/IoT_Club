...
type Form = {
  ...
  reason_for_joining: string; skill_level: Skill['level']; previous_iot_experience: boolean; experience_description: string;
...
const initial: Form = {
  ...
  reason_for_joining: '', skill_level: 'BEGINNER', previous_iot_experience: false, experience_description: '',
...
export default function RegistrationForm({ collegeEmail }: { collegeEmail: string }) {
  ...
  function validateCurrent() {
    if (step === 1 && (!form.full_name.trim() || !form.date_of_birth || !/^[0-9+() -]{7,20}$/.test(form.mobile_number) || !/^\S+@\S+\.\S+$/.test(form.personal_email))) return 'Complete all required personal details.'
    ...
    if (step === 3 && (!form.reason_for_joining.trim() || selectedInterests.length === 0 || (form.previous_iot_experience && !form.experience_description.trim()))) return 'Complete the required IoT and skills details.'
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = { ...form, college_email: collegeEmail, year_of_study: Number(form.year_of_study), semester: Number(form.semester), interests: selectedInterests, skills: selectedSkills }
  ...
  return <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 text-slate-800"><div className="mb-6"><p className="text-xs font-mono text-emerald-700">MEMBERSHIP APPLICATION</p><h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900">IoT Club Registration</h1><p className="text-xs text-slate-500 mt-1">Your application will be reviewed before membership access is granted.</p></div><div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-6 sm:p-8 space-y-6"><div className="flex gap-2" aria-label={`Step ${step} of 5`}>{['Personal', 'Academic', 'IoT & Skills', 'Account', 'Review'].map((name, index) => <div key={name} className={`flex-1 text-center text-[10px] sm:text-xs rounded-xl py-2 ${index + 1 === step ? 'bg-emerald-100 text-emerald-800 font-bold' : 'bg-slate-50 text-slate-500'}`}>{name}</div>)}</div><form onSubmit={submit} className="space-y-5">
    ...
    {step === 5 && <div className="space-y-4 text-xs"><div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-1"><p><strong>Name:</strong> {form.full_name}</p><p><strong>College email:</strong> {collegeEmail}</p><p><strong>Personal email:</strong> {form.personal_email}</p><p><strong>Register number:</strong> {form.register_number}</p><p><strong>Department / degree:</strong> {form.department} / {form.degree_programme}</p><p><strong>Year / semester:</strong> {form.year_of_study} / {form.semester}</p><p><strong>Interests:</strong> {selectedInterests.join(', ')}</p><p><strong>Skill level:</strong> {form.skill_level}</p></div>{([['consent_accuracy', 'I confirm that the information I supplied is accurate.'], ['consent_rules', 'I agree to the IoT Club rules and guidelines.'], ['consent_data_use', 'I agree to academic, membership, and administrative use of my application data.']] as const).map(([key, label]) => <label key={key} className="block text-slate-700"><input type="checkbox" checked={form[key]} onChange={event => update(key, event.target.checked)} className="mr-2" />{label}</label>)}</div>}
