-- Future applications use their confirmed Supabase account email as the sole contact.
...
begin
  ...
  -- The confirmed Auth address is the only required contact email. Ignore legacy
  -- client-provided email fields so a caller cannot attach another person's address.
  college_email_value := case when lower(actor_email) like '%@siet.ac.in'
    then lower(actor_email) else null end;
  personal_email_value := case when lower(actor_email) not like '%@siet.ac.in'
    then lower(actor_email) else null end;
