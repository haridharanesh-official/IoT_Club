-- Future applications use one confirmed Supabase account email as the sole contact.
-- The applicant explicitly identifies it as PERSONAL or COLLEGE; no domain heuristic is used.
-- Historical profile rows remain unchanged.
...
begin
  ...
  -- The confirmed Auth address is the only required contact email. The client may
  -- identify it as PERSONAL or COLLEGE, but it cannot attach a different address.
  email_address_value := nullif(lower(trim(payload->>'email_address')), '');
  email_kind_value := upper(trim(coalesce(payload->>'email_kind', '')));

  if email_address_value is null
     or email_address_value <> lower(actor_email)
     or email_kind_value not in ('PERSONAL', 'COLLEGE')
  then
    raise exception 'confirmed account email and email type required' using errcode = '22023';
  end if;

  college_email_value := case when email_kind_value = 'COLLEGE'
    then email_address_value else null end;
  personal_email_value := case when email_kind_value = 'PERSONAL'
    then email_address_value else null end;
